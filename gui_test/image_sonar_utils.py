...
class SensorData():
    def __init__(self):
        folder = "C:/Users/corri/OneDrive/Documents/SonarExperimentData/09-03-2022/09-03-2022/2022-03-09_15-38-06rec20"
        self.camera_folder = f'{folder}/camera'
        self.sonar_folder = f'{folder}/sonar'
        self.sonar_params = {"range": 10.000000, "gain": 47.000000}
    ...
    def get_pair(self, idx):
        ...
        sonar = cv2.imread(f"{self.sonar_folder}/{current_pair.sonarfile}")
...
def raw_sonar_to_rectangular(image, range_m, wide=False):
    ...
    for y in range(height):
        for x in range(width):
            if masked[y][x][0] != 0: 
                r, th = get_polar_coords(x, y, x_center, 507) #892***
                rscale = range_bins/507 #892***
                ...
                rectangular[int(row)][int(col)] = masked[y][x][0]
...
def init_charuco_sonar():
    ...
    # For 8x11 target {'A1': 0, 'A2': 1, 'A3': 5, 'A4': 6, 'A5': 11,
                    #  'B1': 3, 'B2': 4, 'B3': 9, 'B4': 10, 'B5': 15,  # fmt: skip
                    #  'C1': 27, 'C2': 28, 'C3': 33, 'C4': 38, 'C5': 39,
                    #  'D1': 31, 'D2': 32, 'D3': 37, 'D4': 42, 'D5': 43}
